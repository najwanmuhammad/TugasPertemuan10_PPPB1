...
import com.example.tugaspertemuan10.databinding.FragmentSelectTicketBinding
...
class SelectTicketFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding){
            val arrayTicketTypes = resources.getStringArray(R.array.ticket_types)
            val ticketAdapter = ArrayAdapter(requireContext(), android.R.layout.simple_spinner_item, arrayTicketTypes)
            ticketAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
            spinnerTicket.adapter = ticketAdapter

            btnDone.setOnClickListener {
                findNavController().apply {
                    previousBackStackEntry?.savedStateHandle?.set("ticketType", spinnerTicket.selectedItem.toString())
                }.navigateUp()
            }
        }
    }
